...
export class PriorityQueue {
    ...
    deleteById(taskId) {
        this.order = this.order.filter(item => item.value.taskId !== taskId);

        for (const arr of this.priorityMap.values()) {
            for (let i = 0; i < arr.length; i++) {
                if (arr[i].value.taskId === taskId) {
                    arr.splice(i, 1);
                    break;
                }
            }
        ...
        for (const [key, arr] of this.priorityMap.entries()) {
            if (arr.length === 0) {
    ...
    removeDone() {
        this.order = this.order.filter(item => !item.value.done);
    }   
}
